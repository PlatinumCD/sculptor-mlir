...
| `sculptor-plan-core-scratchpad` | One extracted, non-finalized core plus an opt-in Platform v0.2 capacity contract. | One deterministic resident producer-consumer region, lifetime-aware scratchpad offsets, and fixed-width DMA descriptor metadata. Omitting the pass preserves workspace-only behavior. |
