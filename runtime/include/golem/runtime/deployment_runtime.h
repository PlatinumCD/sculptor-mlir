...
struct DeploymentProfile {
  ...
  uint64_t readiness_decrements = 0;
  uint64_t direct_index_hits = 0;
  uint64_t compatibility_search_calls = 0;
  uint64_t maximum_ready_queue_occupancy = 0;
  uint64_t route_ready_to_task_start_events = 0;
  uint64_t route_ready_to_task_start_cycles = 0;
  uint64_t task_finish_to_nic_submit_events = 0;
  uint64_t task_finish_to_nic_submit_cycles = 0;
};
...
using EmitTaskTrace = void (*)(void *context, TaskTraceEvent event,
                               uint32_t task_id, ExecutionId execution_id);
...
public:
  DeploymentRuntime(TileABI abi, RoutedWordTransport transport,
                    ...
                    HeapProfile *heap_profile = nullptr) noexcept;
...
private:
  ...
  bool bindRouteInputView(const Route &route) noexcept;
  uint8_t *incomingRouteDestination(const Route &route) noexcept;
  uint8_t *
  incomingRouteSegmentDestination(const Route &route,
                                  const MemorySegment &segment) noexcept;
  const uint8_t *outgoingRouteSource(const Route &route) const noexcept;
  const uint8_t *
  outgoingRouteSegmentSource(const Route &route,
                             const MemorySegment &segment) const noexcept;
  void completeIncomingRoute(const Route &route) noexcept;
  void updateAssemblyReadiness(uint32_t route_id) noexcept;
  ...
  bool staticRuntimeEnabled() const noexcept;
  bool notifyConsumers(uint32_t offset, uint32_t count,
                       bool route_arrival = false) noexcept;
  bool markResourceReady(uint32_t local_slot,
                         bool route_arrival = false) noexcept;
  bool notifyTaskComplete(uint32_t task_index) noexcept;
  bool enqueueReadyTask(uint32_t task_index,
                        bool route_arrival = false) noexcept;
  bool popReadyTask(uint32_t *task_index) noexcept;
  bool popReadyTaskWithoutTransmitConflict(uint32_t *task_index) noexcept;
  void restoreReadyHeap(uint32_t index) noexcept;
  bool routeConflictsWithOutput(const Route &route, uint32_t output_slot,
                                uintptr_t output_start,
                                uintptr_t output_finish) const noexcept;
  bool selectNextTaskRoute() noexcept;
  bool
  taskOutputConflictsWithPendingTransmit(uint32_t task_index) const noexcept;
  ...
  void setInvalidFrame(InvalidFrameReason reason, uint32_t source_tile,
                       uint32_t route_id, uint32_t receive_phase,
                       uint64_t expected, uint64_t actual) noexcept;
  void setError(DeploymentError error) noexcept;
  uint64_t profileCycle() const noexcept;
  void recordProfileStep(ProfileActivity activity,
                         uint64_t start_cycle) noexcept;
  void emitTaskTrace(TaskTraceEvent event, uint32_t task_id) noexcept;
  ...
  uint32_t transmit_segment_index_;
  uint32_t transmit_segment_word_offset_;
  uint32_t transmit_route_limit_;
  uint32_t complete_outgoing_route_count_;
  uint32_t complete_incoming_route_count_;
  uint32_t *task_readiness_;
  bool *task_queued_;
  uint32_t *ready_tasks_;
  uint32_t ready_task_count_;
  uint64_t *task_ready_cycles_;
  bool *task_ready_from_route_;
  uint64_t *task_finish_cycles_;
  bool *task_nic_submitted_;
  DeploymentProfile *profile_;
  DeploymentTrace *trace_;
  HeapProfile *heap_profile_;
};
