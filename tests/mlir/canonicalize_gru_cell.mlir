...
module {
  ...
  // PIPE: %[[CALL:.*]] = call @grucellwbias_0(%arg0, %arg1) : (tensor<1x4xf32>, tensor<1x3xf32>) -> tensor<1x3xf32>
  // PIPE: return %[[CALL]]
  func.func @forward(%arg0: tensor<1x4xf32>, %arg1: tensor<1x3xf32>)
    ...
    %b_ih = arith.constant dense<2.000000e+00> : tensor<9xf32>
    %w_hh = arith.constant dense<3.000000e+00> : tensor<9x3xf32>
    ...
    %ih = linalg.generic {
        ...
        ins(%ih_mm, %b_ih : tensor<1x9xf32>, tensor<9xf32>)
        outs(%pre_empty : tensor<1x9xf32>) {
    ...
    %hh = linalg.generic {
        indexing_maps = [#map, #bias, #map],
        iterator_types = ["parallel", "parallel"]}
        ins(%hh_mm, %b_hh : tensor<1x9xf32>, tensor<9xf32>)
        outs(%pre_empty : tensor<1x9xf32>) {
    ^bb0(%in: f32, %bias_in: f32, %out: f32):
      %sum = arith.addf %in, %bias_in : f32
      linalg.yield %sum : f32
  ...
  // PIPE-LABEL: func.func @grucellwbias_0
  // PIPE-SAME: (%arg0: tensor<1x4xf32>, %arg1: tensor<1x3xf32>)
  // PIPE-SAME: attributes {layer_type = "gru_cell_w_bias"}
