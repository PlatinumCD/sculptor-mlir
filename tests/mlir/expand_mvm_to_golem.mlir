...
module {
  ...
  // CHECK: } {sculptor.source_resource = "torch_tensor_3_4_torch.float32", sculptor.tile = [0, 0], sculptor.tile_grid = [2, 2]} : () -> !sculptor.logical.array
  ...
  // CHECK: } {sculptor.vector_tile = 0 : i64, sculptor.vector_tile_grid = 2 : i64} : (tensor<1x4xf32>) -> tensor<1x3xf32>
  ...
  // CHECK: } {sculptor.vector_tile = 1 : i64, sculptor.vector_tile_grid = 2 : i64} : (tensor<1x4xf32>) -> tensor<1x3xf32>
  ...
  // CHECK: } {sculptor.source_resource = "torch_tensor_3_4_torch.float32", sculptor.tile = [0, 0], sculptor.tile_grid = [2, 2], sculptor.vector_tile = 0 : i64, sculptor.vector_tile_grid = 2 : i64} : (tensor<1x3xf32>, !sculptor.logical.array) -> tensor<1x2xf32>
  ...
  // CHECK: ^bb0(%[[R00:.*]]: tensor<1x2xf32>, %[[R01:.*]]: tensor<1x2xf32>, %[[R10:.*]]: tensor<1x2xf32>, %[[R11:.*]]: tensor<1x2xf32>):
  ...
  // CHECK: %[[INIT1:.*]] = tensor.empty() : tensor<1x2xf32>
  // CHECK: %[[ROW1:.*]] = linalg.add ins(%[[R10]], %[[R11]] : tensor<1x2xf32>, tensor<1x2xf32>) outs(%[[INIT1]] : tensor<1x2xf32>) -> tensor<1x2xf32>
  // CHECK: %[[WIDE:.*]] = tensor.concat dim(1) %[[ROW0]], %[[ROW1]] : (tensor<1x2xf32>, tensor<1x2xf32>) -> tensor<1x4xf32>
  // CHECK: %[[RESULT:.*]] = tensor.extract_slice %[[WIDE]][0, 0] [1, 3] [1, 1] : tensor<1x4xf32> to tensor<1x3xf32>
