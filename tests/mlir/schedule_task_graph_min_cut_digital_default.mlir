// RUN: sculptor-mlir-opt %s --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=snake" | FileCheck %s --check-prefix=DEFAULT
// RUN: sculptor-mlir-opt %s --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=random" | FileCheck %s --check-prefix=DEFAULT
...
module {
  ...
  func.func private @generate_task_graph() -> !sculptor.task_graph {
    ...
    %tmp0 = sculptor.task_graph.temporary %graph {sculptor.runtime.byte_size = 100 : i64} : !sculptor.task_graph -> !sculptor.task_resource<tensor<1x2xf32>>
    %tmp1 = sculptor.task_graph.temporary %graph {sculptor.runtime.byte_size = 4 : i64} : !sculptor.task_graph -> !sculptor.task_resource<tensor<1x1xf32>>
    %tmp2 = sculptor.task_graph.temporary %graph {sculptor.runtime.byte_size = 100 : i64} : !sculptor.task_graph -> !sculptor.task_resource<tensor<1x2xf32>>
