// RUN: sculptor-mlir-opt %s --sculptor-schedule-task-graph="cores=2 arrays-per-core=2 topology=mesh mesh-rows=1 mesh-cols=2 schedule=boundary-aware-cut-optimized" | FileCheck %s
