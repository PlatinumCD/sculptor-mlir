// RUN: sculptor-mlir-opt %s --sculptor-convert-layers --sculptor-expand-mvm-to-golem="array-rows=1 array-cols=4" --sculptor-materialize-tasks --sculptor-assemble-task-graph --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=1 arrays-per-core=3 topology=mesh mesh-rows=1 mesh-cols=1 schedule=snake" --sculptor-optimize-task-graph="patterns=streaming-convolution require-change=true" --sculptor-fuse-task-graph --sculptor-analyze-task-graph-timing | FileCheck %s --check-prefix=STREAM --implicit-check-not="tensor<9x9xf32>" --implicit-check-not="task_kind = \"digital.conv_patch\"" --implicit-check-not="task_kind = \"sculptor.conv_tile_mvm\"" --implicit-check-not="task_kind = \"digital.tile_recombine\"" --implicit-check-not="task_kind = \"digital.bias_add\""
// RUN: sculptor-mlir-opt %s --sculptor-convert-layers --sculptor-expand-mvm-to-golem="array-rows=1 array-cols=4" --sculptor-materialize-tasks --sculptor-assemble-task-graph --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=1 arrays-per-core=3 topology=mesh mesh-rows=1 mesh-cols=1 schedule=snake" --sculptor-optimize-task-graph --sculptor-fuse-task-graph --sculptor-analyze-task-graph-timing --sculptor-lower-golem-to-llvm-shims --sculptor-finalize-task-graph-resources | FileCheck %s --check-prefix=SHIM --implicit-check-not="!sculptor.logical.array" --implicit-check-not="sculptor.array."
...
// RUN: not sculptor-mlir-opt %s --sculptor-convert-layers --sculptor-expand-mvm-to-golem="array-rows=1 array-cols=4" --sculptor-materialize-tasks --sculptor-assemble-task-graph --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=3 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=3 schedule=snake" --sculptor-optimize-task-graph="require-change=true" 2>&1 | FileCheck %s --check-prefix=DISTRIBUTED
...
// STREAM-SAME: sculptor.runtime.analog_store_bytes = 108 : i64
...
// STREAM-SAME: sculptor.timing.analog_store_latency_ns = 4.000000e+00 : f64
