...
// RUN: sculptor-mlir-opt %s --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=annealing annealing-initial-schedule=snake annealing-move-set=segment-reverse annealing-steps-per-temperature=3 annealing-plateau-patience=2 annealing-minimum-epochs=2 annealing-plateau-acceptance-rate=1 annealing-maximum-evaluations=1000 annealing-maximum-runtime-seconds=0" | FileCheck %s --check-prefix=ANNEALING-PLATEAU
// RUN: sculptor-mlir-opt %s --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=annealing annealing-initial-schedule=snake annealing-move-set=segment-reverse annealing-steps-per-temperature=3 annealing-plateau-patience=2 annealing-minimum-epochs=2 annealing-plateau-acceptance-rate=1 annealing-maximum-evaluations=1000 annealing-maximum-runtime-seconds=0 summary-output=%t.annealing-summary.csv" -o /dev/null
// RUN: FileCheck %s --check-prefix=ANNEALING-SUMMARY --input-file=%t.annealing-summary.csv
// RUN: sculptor-mlir-opt %s --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=annealing annealing-initial-schedule=snake annealing-move-set=segment-reverse annealing-steps-per-temperature=3 annealing-plateau-patience=100 annealing-minimum-epochs=100 annealing-maximum-evaluations=5 annealing-maximum-runtime-seconds=0" | FileCheck %s --check-prefix=ANNEALING-MAX-EVALUATIONS
...
// RUN: sculptor-mlir-opt %s --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=greedy greedy-heuristic=transfer-cost,link-pressure,lookahead=1,beam=1,scope=cardinal" | FileCheck %s --check-prefix=LINK-PRESSURE
// RUN: sculptor-mlir-opt %s --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=greedy-timing greedy-heuristic=transfer-cost,link-pressure,lookahead=1,beam=1,scope=cardinal" | FileCheck %s --check-prefix=LINK-PRESSURE-TIMING
...
// RUN: not sculptor-mlir-opt %s --sculptor-build-task-graph-islands --sculptor-analyze-task-graph-timing --sculptor-schedule-task-graph="cores=2 arrays-per-core=1 topology=mesh mesh-rows=1 mesh-cols=2 schedule=annealing annealing-plateau-patience=0" 2>&1 | FileCheck %s --check-prefix=INVALID-ANNEALING-PLATEAU
...
// ANNEALING-PLATEAU-LABEL: func.func private @generate_task_graph
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_epochs = 2 : i64
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_evaluations = 6 : i64
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_maximum_evaluations = 1000 : i64
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_minimum_epochs = 2 : i64
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_plateau_acceptance_rate = 1.000000e+00 : f64
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_plateau_patience = 2 : i64
// ANNEALING-PLATEAU-DAG: sculptor.schedule.annealing_stop_reason = "plateau"

// ANNEALING-MAX-EVALUATIONS-LABEL: func.func private @generate_task_graph
// ANNEALING-MAX-EVALUATIONS-DAG: sculptor.schedule.annealing_epochs = 1 : i64
// ANNEALING-MAX-EVALUATIONS-DAG: sculptor.schedule.annealing_evaluations = 5 : i64
// ANNEALING-MAX-EVALUATIONS-DAG: sculptor.schedule.annealing_stop_reason = "maximum-evaluations"

// ANNEALING-SUMMARY: generate_task_graph,annealing,2,1,1,2,1,1,diagonal,transfer-cost,4,3,2,0,8,8,1.000000e+00,0,8,plateau,2,6,8,8,0.000000e+00,{{[0-9.e+-]+}}
...
// INVALID-ANNEALING-PLATEAU: expected Sculptor annealing plateau patience to be positive
...
// LINK-PRESSURE-LABEL: func.func private @generate_task_graph
// LINK-PRESSURE-SAME: sculptor.schedule.graph_score = 8 : i64
// LINK-PRESSURE-SAME: sculptor.schedule.greedy_heuristic = "transfer-cost,link-pressure,lookahead=1,beam=1,scope=cardinal"
// LINK-PRESSURE-SAME: sculptor.schedule.total_transfer_cost = 8 : i64

// LINK-PRESSURE-TIMING-LABEL: func.func private @generate_task_graph
// LINK-PRESSURE-TIMING-SAME: sculptor.schedule.graph_score = 8 : i64
// LINK-PRESSURE-TIMING-SAME: sculptor.schedule.greedy_heuristic = "transfer-cost,link-pressure,lookahead=1,beam=1,scope=cardinal"
// LINK-PRESSURE-TIMING-SAME: sculptor.schedule.total_transfer_cost = 8 : i64
