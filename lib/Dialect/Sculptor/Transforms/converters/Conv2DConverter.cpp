...
static mlir::FailureOr<Conv2DConvolutionAttrs>
getSupportedConvolutionAttrs(NNConv2DOp convOp) {
  ...
  return Conv2DConvolutionAttrs{strides[0], strides[1],   padding[0],
                                padding[1], dilations[0], dilations[1]};
}

static mlir::FailureOr<Conv2DShapeInfo> getValidatedShapeInfo(
    mlir::RankedTensorType inputTy,
    std::optional<mlir::RankedTensorType> biasTy,
    mlir::RankedTensorType filterRank4Ty, mlir::RankedTensorType filterRank2Ty,
    mlir::RankedTensorType outputTy, const Conv2DConvolutionAttrs &attrs) {
  ...
  Conv2DShapeInfo shapeInfo{
      ...
      inputShape[3],  filterShape[0], filterShape[2],
      filterShape[3], outputShape[2], outputShape[3],
  };
...
matchSupportedConv2D(NNConv2DOp convOp, mlir::RewriterBase &rewriter) {
  ...
  mlir::RankedTensorType filterRank2Ty =
      buildFlattenedTensorType(filterRank4Ty);
  auto shapeInfo =
      getValidatedShapeInfo(inputTy, biasMatch->biasTy, filterRank4Ty,
...
static Conv2DLoweringState buildLoweringState(const Conv2DMatch &match) {
...
static PreparedFilter prepareFilter(Conv2DMatch &match) {
...
static mlir::Value
buildPatchPreparationStage(mlir::OpBuilder &builder, const Conv2DMatch &match,
                            const Conv2DLoweringState &state) {
  ...
  mlir::Value patches =
      buildPatchSequence(builder, state, match.activation);
  scope.annotate();
  return patches;
}
...
static mlir::Value buildMVMSequenceStage(mlir::OpBuilder &builder,
...
static mlir::Value buildOutputAssemblyStage(mlir::OpBuilder &builder,
                                             PreparedBias &preparedBias,
                                             const Conv2DLoweringState &state,
  ...
  mlir::sculptor::SemanticOperationScope scope(
      builder, kind, "conv2d_output_assembly");
...
static mlir::Value emitLoopedConvolution(mlir::RewriterBase &rewriter,
  ...
  mlir::Value patchSequence =
      buildPatchPreparationStage(rewriter, match, state);
  mlir::Value mvmSequence =
      buildMVMSequenceStage(rewriter, preparedFilter, state, patchSequence);
  return buildOutputAssemblyStage(rewriter, preparedBias, state, mvmSequence);
}
...
static mlir::LogicalResult lowerConv2DOp(NNConv2DOp layerOp,
                                         mlir::RewriterBase &rewriter) {
  auto match = matchSupportedConv2D(layerOp, rewriter);
  if (failed(match))
    return mlir::failure();

  Conv2DLoweringState state = buildLoweringState(*match);
...
public:
  ...
  void lowerToMVM(mlir::func::FuncOp func) const override {
    ...
    auto layerOp = nn_layer_match::matchSingleNNLayerOp<NNConv2DOp>(func);
    if (failed(layerOp))
      return;

    if (!nn_layer_match::hasLayerTypeMatchingBias(
            func, "conv2d", "conv2d_w_bias", (*layerOp).getHasBias()))
      return;

    (void)lowerConv2DOp(*layerOp, rewriter);
  }
};
