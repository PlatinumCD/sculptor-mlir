...
// Dispatches each extracted sculptor.nn layer called by forward to the
// converter that lowers its layer_type to sculptor.mvm.
void ConvertLayersPass::runOnOperation() {
  ...
  mlir::sculptor::registerGRUCellConverter(converters, converterMap,
                                           &getContext());
  mlir::sculptor::registerGRUConverter(converters, converterMap, &getContext());
  mlir::sculptor::registerRNNConverter(converters, converterMap, &getContext());
  mlir::sculptor::registerTransformerBlockConverter(converters, converterMap,
                                                    &getContext());
  ...
  for (mlir::func::FuncOp func : getOperation().getOps<mlir::func::FuncOp>()) {
    ...
    for (mlir::func::CallOp call : forwardCalls) {
      ...
      const mlir::sculptor::LayerToMVMConverter *converter =
          converterIt->second;
