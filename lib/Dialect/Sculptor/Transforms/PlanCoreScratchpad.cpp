...
std::optional<unsigned> findUniqueNextTask(
  ...
  if (!next || mlir::sculptor::task_graph::isMatrixSetupTask(tasks[*next]))
    return std::nullopt;
  return next;
}
...
void PlanCoreScratchpadPass::runOnOperation() {
  ...
  if (!module->hasAttr(runtime_attrs::kTaskCoreIdAttrName)) {
  ...
  for (Operation &op : graph.getBody().front()) {
    if (op.hasAttr(runtime_attrs::kResourceSlotAttrName) ||
        op.hasAttr(runtime_attrs::kTaskIndexAttrName)) {
  ...
  for (unsigned start = 0; start < tasks.size(); ++start) {
    if (task_graph::isMatrixSetupTask(tasks[start]))
      continue;
...
void registerPlanCoreScratchpadPass() {
  PassRegistration<PlanCoreScratchpadPass>();
}
