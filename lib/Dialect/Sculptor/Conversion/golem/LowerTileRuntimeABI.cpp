...
using ConvertedInputIndexMap = llvm::SmallVector<std::optional<unsigned>, 8>;

constexpr llvm::StringLiteral kInputResourceIdsAttrName =
...
getArrayBinding(mlir::ModuleOp module, mlir::sculptor::TaskCreateOp taskOp) {
  mlir::FailureOr<int64_t> physicalArrayId = getTaskOrCalleeI64Attr(
      module, taskOp, tile_runtime_attrs::kTaskPhysicalArrayIdAttrName);
  mlir::FailureOr<int64_t> localArrayId = getTaskOrCalleeI64Attr(
      module, taskOp, tile_runtime_attrs::kTaskLocalArrayIdAttrName);
...
getOrderedArrayBindingsAttr(mlir::ModuleOp module,
  ...
  if (taskBindings) {
    if (!calleeBindings)
      (*callee)->setAttr(tile_runtime_attrs::kTaskArrayBindingsAttrName,
                         taskBindings);
    return std::optional<mlir::ArrayAttr>(taskBindings);
  }
  if (calleeBindings) {
    taskOp->setAttr(tile_runtime_attrs::kTaskArrayBindingsAttrName, calleeBindings);
...
parseOrderedArrayBindings(mlir::ModuleOp module,
  ...
  for (mlir::Attribute attribute : **bindingsAttr) {
    ...
    auto physicalArrayAttr = dictionary.getAs<mlir::IntegerAttr>(
        tile_runtime_attrs::kArrayBindingPhysicalIdFieldName);
    auto localArrayAttr = dictionary.getAs<mlir::IntegerAttr>(
        tile_runtime_attrs::kArrayBindingLocalIdFieldName);
...
mlir::LogicalResult remapCalleeInputResourceIds(
    mlir::sculptor::TaskCreateOp taskOp, mlir::func::FuncOp callee,
    llvm::ArrayRef<std::optional<unsigned>> convertedInputIndices) {
  auto resourceIds =
      callee->getAttrOfType<mlir::ArrayAttr>(kInputResourceIdsAttrName);
  if (!resourceIds)
    return mlir::success();
  if (resourceIds.size() != convertedInputIndices.size()) {
    return taskOp.emitOpError("expected callee '")
           << callee.getSymName() << "' attribute '"
           << kInputResourceIdsAttrName
           << "' to contain one entry per pre-conversion input";
  }

  llvm::SmallVector<mlir::Attribute, 8> retainedResourceIds;
  retainedResourceIds.reserve(resourceIds.size());
  for (auto indexedResource : llvm::enumerate(resourceIds)) {
    if (convertedInputIndices[indexedResource.index()])
      retainedResourceIds.push_back(indexedResource.value());
  }
  callee->setAttr(
      kInputResourceIdsAttrName,
      mlir::ArrayAttr::get(callee.getContext(), retainedResourceIds));
  return mlir::success();
}

mlir::LogicalResult
remapDeploymentRouteArray(mlir::ModuleOp module, llvm::StringRef attributeName,
                          const llvm::DenseMap<int64_t, ConvertedInputIndexMap>
                              &convertedInputIndicesByTask) {
  auto routes = module->getAttrOfType<mlir::ArrayAttr>(attributeName);
  if (!routes || convertedInputIndicesByTask.empty())
    return mlir::success();

  llvm::SmallVector<mlir::Attribute, 8> remappedRoutes;
  remappedRoutes.reserve(routes.size());
  bool changed = false;
  mlir::Builder builder(module.getContext());
  for (mlir::Attribute attribute : routes) {
    auto route = mlir::dyn_cast<mlir::sculptor::DeploymentRouteAttr>(attribute);
    if (!route) {
      remappedRoutes.push_back(attribute);
      continue;
    }

    auto task =
        convertedInputIndicesByTask.find(route.getDestinationTask().getInt());
    if (task == convertedInputIndicesByTask.end()) {
      remappedRoutes.push_back(route);
      continue;
    }

    int64_t oldInputIndex = route.getDestinationInput().getInt();
    if (oldInputIndex < 0 ||
        static_cast<uint64_t>(oldInputIndex) >= task->second.size()) {
      module.emitError("deployment route ")
          << route.getId().getInt() << " targets input " << oldInputIndex
          << " outside the pre-conversion input range of task "
          << route.getDestinationTask().getInt();
      return mlir::failure();
    }

    std::optional<unsigned> newInputIndex = task->second[oldInputIndex];
    if (!newInputIndex) {
      module.emitError("deployment route ")
          << route.getId().getInt()
          << " targets a logical-array input removed from task "
          << route.getDestinationTask().getInt();
      return mlir::failure();
    }
    if (*newInputIndex == static_cast<unsigned>(oldInputIndex)) {
      remappedRoutes.push_back(route);
      continue;
    }

    changed = true;
    remappedRoutes.push_back(mlir::sculptor::DeploymentRouteAttr::get(
        module.getContext(), route.getId(), route.getSourceCore(),
        route.getSourceTask(), route.getSourceOutput(),
        route.getDestinationCore(), route.getDestinationTask(),
        builder.getI64IntegerAttr(*newInputIndex), route.getResourceId(),
        route.getByteSize()));
  }

  if (changed)
    module->setAttr(attributeName, builder.getArrayAttr(remappedRoutes));
  return mlir::success();
}

mlir::LogicalResult
remapDeploymentInputPorts(mlir::ModuleOp module,
                          const llvm::DenseMap<int64_t, ConvertedInputIndexMap>
                              &convertedInputIndicesByTask) {
  if (mlir::failed(remapDeploymentRouteArray(module,
                                             deployment_attrs::kRoutesAttrName,
                                             convertedInputIndicesByTask)) ||
      mlir::failed(remapDeploymentRouteArray(
          module, deployment_attrs::kIncomingRoutesAttrName,
          convertedInputIndicesByTask)) ||
      mlir::failed(remapDeploymentRouteArray(
          module, deployment_attrs::kOutgoingRoutesAttrName,
          convertedInputIndicesByTask)))
    return mlir::failure();
  return mlir::success();
}
...
mlir::LogicalResult
lowerTileRuntimeFunctionABI(mlir::ModuleOp module,
                          mlir::func::FuncOp taskGraphFunc,
                          mlir::TypeConverter &typeConverter) {
  bool hadExecutionPlan =
      taskGraphFunc->hasAttr(tile_runtime_attrs::kTaskGraphResourceCountAttrName);

  llvm::DenseMap<mlir::Value, mlir::sculptor::TaskCreateOp> producerByResource;
  llvm::DenseMap<int64_t, ConvertedInputIndexMap> convertedInputIndicesByTask;
  ...
  for (mlir::sculptor::TaskCreateOp taskOp :
    ...
    ConvertedInputIndexMap convertedInputIndices(taskOp.getInputs().size(),
                                                 std::nullopt);
    ...
    for (auto indexedInput : llvm::enumerate(taskOp.getInputs())) {
      ...
      if (!isLogicalArrayResource(input)) {
        convertedInputIndices[indexedInput.index()] = retainedInputs.size();
    ...
    mlir::FailureOr<mlir::func::FuncOp> callee =
        lookupTaskCallee(module, taskOp);
    if (mlir::failed(callee) || mlir::failed(remapCalleeInputResourceIds(
                                    taskOp, *callee, convertedInputIndices)))
      return mlir::failure();

    if (logicalInputCount != 0) {
      auto taskId = taskOp->getAttrOfType<mlir::IntegerAttr>(
          deployment_attrs::kGlobalTaskIdAttrName);
      auto calleeTaskId = (*callee)->getAttrOfType<mlir::IntegerAttr>(
          deployment_attrs::kGlobalTaskIdAttrName);
      if (taskId && calleeTaskId && taskId.getInt() != calleeTaskId.getInt()) {
        return taskOp.emitOpError("expected task and callee '")
               << (*callee).getSymName() << "' to agree on '"
               << deployment_attrs::kGlobalTaskIdAttrName << "'";
      }
      if (!taskId)
        taskId = calleeTaskId;
      if (taskId) {
        auto inserted = convertedInputIndicesByTask.try_emplace(
            taskId.getInt(), convertedInputIndices);
        if (!inserted.second) {
          return taskOp.emitOpError("duplicate deployment global task ID ")
                 << taskId.getInt();
        }
      }
    }
    ...
    if (!orderedBindings->empty()) {
      taskOp->removeAttr(tile_runtime_attrs::kTaskArrayBindingsAttrName);
      (*callee)->removeAttr(tile_runtime_attrs::kTaskArrayBindingsAttrName);
    }

    if (convertedResultIndices->empty()) {
      taskOp->removeAttr(tile_runtime_attrs::kTaskResultIndicesAttrName);
    } else {
      mlir::Builder builder(taskOp.getContext());
      taskOp->setAttr(tile_runtime_attrs::kTaskResultIndicesAttrName,
                      builder.getI64ArrayAttr(*convertedResultIndices));
    }
  }

  if (mlir::failed(
          remapDeploymentInputPorts(module, convertedInputIndicesByTask)))
    return mlir::failure();
  ...
  if (hadExecutionPlan &&
      mlir::failed(
          mlir::sculptor::rebuildTileRuntimeLayout(taskGraphFunc))) {
...
LogicalResult lowerTileRuntimeABI(ModuleOp module, TypeConverter &typeConverter) {
  for (func::FuncOp func : module.getOps<func::FuncOp>()) {
    if (!returnsTaskGraph(func))
      continue;
    if (failed(lowerTileRuntimeFunctionABI(module, func, typeConverter)))
      return failure();
  }
  return success();
}
